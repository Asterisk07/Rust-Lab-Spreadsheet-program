...
// Linked list operations using the memory pool
pub fn push_front(head: &mut Option<Rc<RefCell<Node>>>, value: i32) {
    let mut pool = unsafe { &mut crate::list::MEM_POOL };
...
pub fn erase_list(head: &mut Option<Rc<RefCell<Node>>>, value: i32) -> bool {
    if head.is_none() {
        return false;
    }
