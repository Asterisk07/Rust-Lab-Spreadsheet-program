...
pub fn parse_excel_style(input: &str) -> Result<Operation, &'static str> {
    ...
    // Example: A1=42 or B2=C1+D3 or A1=2+3
    if let Some(eq_pos) = trimmed.find('=') {
        ...
        // Case: single cell reference (e.g., A2 = B1)
        if let Ok(idx) = cell_to_index(rhs) {
            println!("Parsed: Set cell {} = {} (as reference)", lhs, rhs);
            return Ok(Operation::SetFormula(row, col, 0, idx, 0)); // function_id 0 for assignment
        }

        // Arithmetic formulas: A1 = X op Y
        let operators = ['+', '-', '*', '/'];
