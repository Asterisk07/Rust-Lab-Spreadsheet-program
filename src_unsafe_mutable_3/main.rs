...
fn main() -> io::Result<()> {
    ...
    let spreadsheet = Rc::new(RefCell::new(sheet::Sheet::new(n, m)));
    ...
    loop {
        if parser_ctx.output_enabled {
            // spreadsheet.display()?;
            spreadsheet.borrow().display()?; // Borrow for display
        }
        ...
        match graph::update_expression(
            ...
            &cmd_info.info,
            &spreadsheet, // Pass the Rc directly
        ) {
            Ok(_) => {}
            Err(_) => set_status_code(StatusCode::CyclicDep),
        }

        // match graph::update_expression(cmd_info.lhs_cell as usize, &cmd_info.info, &mut spreadsheet)
        // {
        //     Ok(_) => {}
        //     Err(_) => set_status_code(StatusCode::CyclicDep),
        // }
